...
const ShiftManagement = () => {
  ...
  // יצירת מערך של תאריכי השבוע הנוכחי
  const [weekDates] = useState(() => {
    const today = new Date();
    const dayOfWeek = today.getDay(); // 0 = ראשון, 1 = שני, וכו'
    const diff = dayOfWeek === 0 ? 0 : -dayOfWeek; // מציאת ההפרש לתחילת השבוע
                          ...
                          <td className="p-2 border">
                            {isEditMode ? (
                              <EditableCell 
                                ...
                                hours={hours}
                                assignments={data.assignments[date.toISOString()][hours]}
                              />
                            ) : (
                              data.assignments[date.toISOString()][hours]?.map((soldier, i) => (
                                <div 
                                  key={i} 
